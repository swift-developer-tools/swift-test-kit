...
diff output produced by ``XCTKAssertEqual(_:_:_:file:line:options:)-func``. 
When the given values are not equal, XCTestKit computes a structural diff and 
formats it as a path-based summary, providing clear and actionable insight into 
where the values differ. Other assertions do not produce diffs, since they 
