...
STKForAll(using: .nonEmptyArray(of: Int.self))
{
    ...
    STKTarget(Double(imbalance))
    
    // Test properties that must hold for any partition.
    STKAssertEqual(left.count + right.count, array.count)
}
```
    ...
    ) async
    {
        switch self
        {
            ...
            case .count:
                ...
                STKAssertEqual(model.count, system.count)
        }
    }
...
await STKForAll
{
    ...
    await STKEventually(timeout: .seconds(2))
    {
        STKAssertEqual(.completed, processor.state)
    }
}
```
